...
class EvaluationCriteriaListSerializer(serializers.ModelSerializer):
    ...
    def get_related_requirements(self, obj):
        """获取关联的需求对象信息"""
        from project.models import Requirement
        requirements = Requirement.objects.filter(evaluation_criteria=obj).select_related('organization',
                                                                                          'publish_people__user')
...
class IndicatorScoreSerializer(serializers.ModelSerializer):
    ...
    indicator_weight = serializers.DecimalField(source='indicator.weight', max_digits=5, decimal_places=2,
                                                read_only=True)
